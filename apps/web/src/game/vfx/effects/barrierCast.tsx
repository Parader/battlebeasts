...
export function BarrierCastEffect({
    ...
    const isHolyBlessing = statusId === "guardiansBlessing";
    ...
    useEffect(() => {
        if (!isHolyBlessing) return;
        burstElementRole("holy", "impact", shot.x, shot.y + 0.16, shot.z);
    }, [isHolyBlessing, shot.key, shot.x, shot.y, shot.z]);
    ...
    useFrame((_, dt) => {
        ...
        if (followId) {
            const local = follow.localSessionId && followId === follow.localSessionId && follow.predictedRef ? follow.predictedRef.current : null;
            if (local) {
                pose.current.x = local.x;
                pose.current.z = local.z;
                pose.current.yaw = local.yaw;
            } else {
                const p = follow.room?.state?.players?.get(followId) as { x?: number; z?: number; yaw?: number } | undefined;
                if (p) {
...
export function GuardiansBlessingEffect({ shot, follow }: { shot: OneShotEffect; follow: VfxFollowContext }) {
    return <BarrierCastEffect shot={shot} follow={follow} statusId="guardiansBlessing" shellColor={BLESSING_GOLD} rimColor={BLESSING_GOLD_HOT} />;
}
