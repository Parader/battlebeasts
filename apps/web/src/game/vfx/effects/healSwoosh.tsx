...
function HealArc({ shot, follow, pose }: { shot: OneShotEffect; follow: VfxFollowContext; pose: ArcPose }) {
    ...
    useFrame(() => {
        ...
        if (shot.followOwnerId) {
            const localPred = follow.localSessionId && shot.followOwnerId === follow.localSessionId && follow.predictedRef ? follow.predictedRef.current : null;
            if (localPred) {
                world.current.x = localPred.x;
                world.current.z = localPred.z;
            } else {
                const p = follow.room?.state?.players?.get(shot.followOwnerId) as { x?: number; z?: number } | undefined;
                if (p) {
...
/** Groove — mesh swooshes with a ParticleWorld leaf impact. */
export function HealSwooshEffect({ shot, follow }: { shot: OneShotEffect; follow: VfxFollowContext }) {
    ...
    useEffect(() => {
        burstElementRole("heal", "impact", shot.x, shot.y + 0.12, shot.z);
        burstElementRole("heal", "trail", shot.x, shot.y + 0.18, shot.z, {
            rate: 0,
            burst: 10,
            life: 0.45,
            dirY: 1.15,
    ...
    useFrame(() => {
        ...
        const fade = softEnvelope(age, GROUND_APPEAR_MS / Math.max(1, shot.life), 0.82);
