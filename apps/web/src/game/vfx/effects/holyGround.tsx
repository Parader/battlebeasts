...
/** Holy Ground — sigil meshes with a rate-scaled ParticleWorld glint field. */
export function HolyGroundEffect({ shot }: { shot: OneShotEffect }) {
    const def = ABILITIES.holyGround;
    const radius = Math.max(1.5, shot.radius ?? def?.radius ?? HOLY_GROUND_CAST.radius);
    const lifeMs = Math.max(1200, shot.life || HOLY_GROUND_CAST.zoneDurationMs + 200);
    ...
    const groundEmitter = useRef<ElementHandle | null>(null);
    ...
    const outerRuneMat = useMemo(() => createRuneMaterial(GOLD, { opacity: 0, spokes: 6 }), []);
    useEffect(
        () => () => {
            runeMat.dispose();
    ...
    useEffect(() => {
        const emitter = spawnElementRole("holy", "ground", shot.x, 0.08, shot.z);
        emitter.setRateScale(0);
        groundEmitter.current = emitter;
        return () => {
            emitter.kill();
            if (groundEmitter.current === emitter) groundEmitter.current = null;
        };
    ...
    useFrame((_, dt) => {
        ...
        groundEmitter.current?.setRateScale(env);
