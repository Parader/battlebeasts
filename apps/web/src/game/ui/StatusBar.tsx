...
export function StatusBar({ room, sessionId }: { room: Room | null; sessionId: string | null }) {
    ...
    return (
        <div className="pointer-events-none absolute inset-x-0 bottom-36 z-20 flex justify-center">
            <div className="flex flex-wrap justify-center gap-1.5 rounded-xl bg-slate-950/70 px-2 py-1.5 ring-1 ring-white/15 backdrop-blur-sm">
                {rows.map((row) => {
                    ...
                    return (
                        <div
                            key={row.statusId}
                            className="relative flex h-9 w-9 flex-col items-center justify-center overflow-hidden rounded-md ring-1 ring-white/30"
                            style={{ backgroundColor: `${def.color}33` }}
                            title={`${def.name}${row.stacks > 1 ? ` x${row.stacks}` : ""}`}
                        >
                            <span className="text-[10px] font-bold text-white drop-shadow">{def.tag}</span>
                            {row.stacks > 1 && (
                                <span className="absolute right-0.5 top-0 text-[9px] font-bold text-white">
                                    {row.stacks}
                                </span>
                            )}
                            <div
